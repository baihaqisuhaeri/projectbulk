...
class Permohonan_kwitansi_model extends CI_Model
{
    ...
    //tabel permohonan kwitansi

    var $table = 'tb_sj';
    var $column_order = array(null, 'id', 'tgl_mohon', 'no_mohon', 'tgl_sj', 'n_cus', 'no_sj', 'jumlah'); //set column field database for datatable orderable
    var $column_search = array('id', 'tgl_mohon', 'no_mohon', 'tgl_sj', 'n_cus', 'no_sj', 'jumlah'); //set column field database for datatable searchable
    var $order = array('no_mohon' => 'desc'); // default order
    public function get_data_tabel_permohonan_kwitansi()
    {
    ...
    {
        $this->get_data_tabel_permohonan_kwitansi();
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    //akhir tabel permohonan kwitansi
    ...
    public function batal_permohonan_kwitansi($id, $data)
    {
        $this->db->where('id', $id);
        $this->db->update('tb_sj', $data);
    }
